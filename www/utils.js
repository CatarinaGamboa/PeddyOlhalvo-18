...
function menuPercursos (){
    hide("welcoming"); show("menuPercursos");hide('confirmarSaidaPerguntas');
...
function menuResultados(){
	hide ('percurso1'); show('resultados'); hide('confirmarSaidaPerguntas');
...
function entrarEmPercurso(){
	...
	document.getElementById('idPerc').innerHTML = nomesPerc[numeroPercAtual];
  ...
  function p4(){
  	menuDesporto();numeroPercAtual = 3;
	passFimPerc = passPercursos[3];
  }
...
*/
  ...
  function continuarResultadosPass(){
	...
	if (passFimInserida == passFimPerc){
		if (numeroPercAtual==3){
			somaDesp(); menuPercursos();
		}else{
