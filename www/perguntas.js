...
var passPercursos = new Array("fim111","fim222","fim333", 'fim444');
...
function escrevePergunta (listaPer, listaIndic){
	...
	if (perguntasRespostas[pergAtual]==null){
		...
		geral = listaPer[pergAtual][alPerg];
		perg = geral.pergunta.texto;
		perguntasRespostas [pergAtual] = listaPer[pergAtual][alPerg];
		respostaJaInserida = "onfocus=''";
	}else{
		geral = perguntasRespostas[pergAtual];
		perg = geral.pergunta.texto;   
	...
	var type = /^\d+$/.test(geral.resposta)?'number':'text';
	var incluir = "<p id='indicacao'>"+listaIndic[pergAtual]+"</p>"+
                  "<p id='pergunta'>"+perg+"</p>"+
                  "<input type='"+type+"'class='respostas' id='resposta"+pergAtual+"'"+respostaJaInserida+">";
...
function inserirResposta (numeroPerg, resp){
	...
	if (resp!=''){
		...
		document.getElementById('indice'+numeroPerg).style.borderColor='#ff6404';
	}
	if (resp == ''){
		document.getElementById('indice'+numeroPerg).style.border='solid 2.5pt';
		document.getElementById('indice'+numeroPerg).style.borderColor='#2b9afa';
	}
}
...
function detalhes(numero){
	...
	var a = '<span style="color:'+cor+';font-weight: bold;font-size: 20pt;">Pergunta '+(numero+1)+'<br></span>'+ 
			...
			'<br><span id="detRespCerta">A resposta correta: <span style="color: #03ff03;">'+res.respostaReal+'</span></span>';
...
var atDesp = 0;
var saveDesp;
var respostasFeitas = 0;


function escreverDesporto(){
	atDesp = 0;
	var inclInd = ' ';
	for(i = 0; i < atividadesDesp.length ;i++){
		inclInd = inclInd + '<button id = "ativ'+i+'" class="ativid" onclick= "openAtiv('+i+')">'+atividadesDesp[i]+'</button>';
	}
	document.getElementById('indiceDesp').innerHTML=inclInd;
	saveDesp = new Array (atividadesDesp.length);
	for (i = 0; i < saveDesp.length; i++){
		saveDesp[i] = '';
	}
	respostasFeitas = 0;
	openAtiv(0);
...
function openAtiv(num){
	//guardar anterior
	
	if (num != atDesp){
		var a = document.getElementById('respostaDesp'+atDesp).value;
		saveDesp[atDesp] = a != ''? a: '';
		respostasFeitas++;
	}
	//escrever
	var val = saveDesp[num]!=''? saveDesp[num] : ''; 
	document.getElementById('introducDesp').innerHTML = descDesp[num]+"<br><input type='number'class='respostas' id='respostaDesp"+num+"' value='"+val+"'>";
	atDesp = num;
	if(respostasFeitas >= atividadesDesp.length)
		show('despConcluir');
	else
		hide('despConcluir');
}
function concDesp(){
	...
	document.getElementById('mensTerminar').innerHTML = 'Terminou a fase do desporto! Parabéns';
	document.getElementById('confirmarSaidaPerguntas').style.top = '0pt';
...
function somaDesp(){
	...
	var count = 0;
	for(i= 0; i < saveDesp.length; i++){
		count += parseInt(saveDesp[i]);
	}
	pontosPercursos[3] = count;
	document.getElementById('confirmarSaidaPerguntas').style.top='-293pt';
	document.getElementById('passFimPercurso').value = '';
	hide('despConcluir');
}
